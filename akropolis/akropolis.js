...
function renderSingleHex3D(x, y, desc, h, isGhost = false) {
    ...
    let p0 = [cx, cy - HEX_R * ISO_RATIO];                  
    let p1 = [cx + HEX_W/2, cy - (HEX_R/2) * ISO_RATIO];    
    let p2 = [cx + HEX_W/2, cy + (HEX_R/2) * ISO_RATIO];    
    let p3 = [cx, cy + HEX_R * ISO_RATIO];                  
    let p4 = [cx - HEX_W/2, cy + (HEX_R/2) * ISO_RATIO];    
    let p5 = [cx - HEX_W/2, cy - (HEX_R/2) * ISO_RATIO];    
...
// Generate the 3D City representation
...
// Generate a 3D view of a construction site tile
globalThis.ui_renderTile3D = function(tileData, isSelected, svgClass) {
    if (!tileData || tileData.length < 3) return '';

    let c0 = getHexCoords(1, -1); 
    let c1 = getHexCoords(0, 0);  
    let c2 = getHexCoords(1, 0);  
...
// Render interactions (ghosts and selectable points)
globalThis.ui_renderInteractions3D = function(validPlacements, ghostHexes, selectedSite) {
    ...
    if (selectedSite >= 0 && ghostHexes && ghostHexes.length > 0) {
        ...
        sortedGhost.forEach(hex => {
            ...
            let baseHeight = existing ? existing.h : 0;
    ...
    if (validPlacements && validPlacements.length > 0) {
        validPlacements.forEach(siteIdx => {
            ...
            // Inline selection handler checking the global drag state
...
// Calculate city bounding box
globalThis.ui_getCityBounds = function(cityState) {
    let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
    
    let gridCenterX = HEX_W * (6 + 0.5 * (6 & 1));
    let gridCenterY = 1.5 * HEX_R * 6 * ISO_RATIO; 
...
/* ==================================================== */
/* ===== CAMERA & CONTROLS (Panzoom)             ====== */
/* ==================================================== */

// Initialize Panzoom and map events
globalThis.ui_initPanzoom = function(containerEl, layerEl) {
    ...
    layerEl.addEventListener('panzoompan', (e) => { 
        // Lock click interactions if user is panning
...
// Reset camera to fit city bounds gracefully
globalThis.ui_resetCam = function(containerEl, pzInstance, cityState) {
    ...
    // Safety unlock for future interactions
